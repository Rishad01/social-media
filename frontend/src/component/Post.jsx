...
function Post(){
  ...
  const fetchPosts = async () => {
    ...
    try {
      const res = await axios.get('social-media-three-puce.vercel.app/bring', {
        headers: {
  ...
  const createPost = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('social-media-three-puce.vercel.app/create/api/posts', { content }, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const deletePost = async (id) => {
    try {
      const token = localStorage.getItem('token');
      console.log(token);
      await axios.delete(`social-media-three-puce.vercel.app/api/posts/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const updatePost = async () => {
    try {
      ...
      const res = await axios.put(`social-media-three-puce.vercel.app/api/posts/${postIdToUpdate}`, updatedPost, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const toggleLike = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`social-media-three-puce.vercel.app/api/posts/${id}/like`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleComment = async (id, commentContent) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`social-media-three-puce.vercel.app/api/posts/${id}/comment`, { text: commentContent }, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
